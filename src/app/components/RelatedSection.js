...
const RelatedSection = ({category, genre}) => {
    const { order, setOrder } = useOrder()
    const [related, setRelated] = useState([])
    const [loading, setloading] = useState(true)
    const [alert, setAlert] = useState(false)
    async function fetchRelatedSection() {
        try{
            const res = await axiosInstance.get(`/${category}?genre=${genre}`)
            setRelated(res.data.data[category])
            setloading(false)
        }
        catch(error){
            console.log(error)
    ...
    return (
        loading ? (
            <div className={`flex flex-col items-start justify-center py-10 px-5 lg:px-20 h-auto mb-5 `}>
                <h1 className='mb-10 xl:text-5xl text-3xl font-medium'>
                    {category === "mangas" ? "Related Mangas" : "Related Comics"}
                </h1>
        ...
        : (
            <>
            <div className={`flex flex-col items-start justify-center py-10 px-5 lg:px-20 h-auto mb-5`}> 
                <h1 className='mb-10 xl:text-5xl text-3xl font-semibold'>{category === "mangas" ? "Related Mangas" : "Related Comics"}</h1>
                <div className='w-full'>
                    <Slider {...settings}>
